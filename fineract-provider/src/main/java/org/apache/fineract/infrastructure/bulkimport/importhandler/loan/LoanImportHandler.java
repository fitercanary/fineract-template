...
public class LoanImportHandler implements ImportHandler {
	...
	private LoanAccountData readLoan(Row row, String locale, String dateFormat) {
		...
		Boolean isTranche = ImportHandlerUtils.readAsBoolean(LoanConstants.IS_TRANCHE_LOAN_COL, row);
		if (Boolean.TRUE.equals(isTranche)) {
			Sheet trancheSheet = workbook.getSheet(TemplatePopulateImportConstants.TRANCH_SHEET_NAME);
			Integer noOfEntries = ImportHandlerUtils.getNumberOfRows(trancheSheet, TemplatePopulateImportConstants.FIRST_COLUMN_INDEX);
			for (int rowIndex = 1; rowIndex <= noOfEntries; rowIndex++) {
				...
				if (externalId.equals(trancheExternalId)) {
					...
					DisbursementData dd = new DisbursementData(expectedDisbursementDate, disbursementDate, tranchePrincipal);
					disbursementData.add(dd);
				}
			}
		}
		statuses.add(status);
		if (loanType != null) {
			if (loanType.equals("individual")) {
				...
				return LoanAccountData.importInstanceIndividual(loanTypeEnumOption, clientId, productId, loanOfficerId, submittedOnDate, fundId,
						...
						row.getRowNum(), externalId, null, charges, linkAccountId, locale, dateFormat, disbursementData);
				...
				return LoanAccountData.importInstanceIndividual(loanTypeEnumOption, clientId, productId, loanOfficerId, submittedOnDate, fundId,
						...
						amortizationEnumOption, interestMethodEnum, interestCalculationPeriodEnum, arrearsTolerance, repaymentStrategyId,
						graceOnPrincipalPayment, graceOnInterestPayment, graceOnInterestCharged, interestChargedFromDate, firstRepaymentOnDate,
						row.getRowNum(), externalId, groupId, charges, linkAccountId, locale, dateFormat, disbursementData);
			} else {
				...
				return LoanAccountData.importInstanceGroup(loanTypeEnumOption, groupIdForGroupLoan, productId, loanOfficerId, submittedOnDate, fundId,
						...
						interestChargedFromDate, firstRepaymentOnDate, row.getRowNum(), externalId, linkAccountId, locale, dateFormat, disbursementData);
			}
		} else {
			return null;
		}
