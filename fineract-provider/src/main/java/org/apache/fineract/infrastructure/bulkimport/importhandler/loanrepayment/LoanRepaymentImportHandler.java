...
public class LoanRepaymentImportHandler implements ImportHandler {
	...
	private LoanTransactionData readLoanRepayment(Row row, String locale, String dateFormat) {
		...
		if (loanaccountInfo != null) {
			...
			if (loanAccountAr[0].startsWith("2")) {
                loanAccountAr[0] = loanAccountAr[0].substring(1, loanAccountAr[0].length() - 1);
            }
		...
		String note = ImportHandlerUtils.readAsString(LoanRepaymentConstants.NOTE_COL, row);
		return LoanTransactionData.importInstance(repaymentAmount, repaymentDate, repaymentTypeId, accountNumber,
				checkNumber, routingCode, receiptNumber, bankNumber, note, loanAccountId, "", row.getRowNum(), locale, dateFormat, externalId);
	}
