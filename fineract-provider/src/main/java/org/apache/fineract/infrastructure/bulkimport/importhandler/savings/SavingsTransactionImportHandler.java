...
public class SavingsTransactionImportHandler implements ImportHandler {
	...
	@Autowired
	public SavingsTransactionImportHandler(SavingsAccountRepository savingsAccountRepository, final PortfolioCommandSourceWritePlatformService
	...
	private SavingsAccountTransactionData readSavingsTransaction(Row row, String locale, String dateFormat) {
		...
		if (savingsAccountIdCheck != null) {
			SavingsAccount savingsAccount = this.savingsAccountRepository.findByAccountNumber(savingsAccountIdCheck);
			savingsAccountId = savingsAccount != null ? savingsAccount.getId().toString() : savingsAccountIdCheck;
		}
