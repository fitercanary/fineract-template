...
public class RecurringDepositTransactionWorkbookPopulator extends AbstractWorkbookPopulator {
    ...
    private void setRules(Sheet worksheet,String dateFormat) {
        ...
        DataValidationConstraint transactionDateConstraint = validationHelper.createDateConstraint
                (DataValidationConstraint.OperatorType.BETWEEN, "=VLOOKUP($C1,$R$2:$U$" +
    ...
    private void setNames(Sheet worksheet) {
        ...
        for(int j = 0; j < clientsWithActiveSavings.size(); j++) {
            ...
            name.setRefersToFormula(TemplatePopulateImportConstants.SAVINGS_TRANSACTION_SHEET_NAME+"!$R$" + clientNameToBeginEndIndexes.get(clientsWithActiveSavings.get(j))[0] + ":$R$" + clientNameToBeginEndIndexes.get(clientsWithActiveSavings.get(j))[1]);
        }
