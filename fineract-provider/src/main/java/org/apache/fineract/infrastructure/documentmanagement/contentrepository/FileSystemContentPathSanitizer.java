...
public class FileSystemContentPathSanitizer implements ContentPathSanitizer {
    ...
    @PostConstruct
    public void init() {
        regexWhitelistPatterns = regexWhitelist.stream().map(Pattern::compile).collect(Collectors.toList());
    }
    ...
    public String sanitize(String path, BufferedInputStream is) {
        try {
            ...
            String sanitizedPath = FilenameUtils.normalize(path, true);

            String fileName = FilenameUtils.getName(sanitizedPath).toLowerCase();

            if (log.isDebugEnabled()) {
                log.debug("Path: {} -> {} ({})", new Object[]{path, sanitizedPath, fileName});
            }
            ...
            String rootFolder = FileSystemContentRepository.FINERACT_BASE_DIR + File.separator +
                    ThreadLocalContextUtil.getTenant().getName().replaceAll(" ", "").trim();

            if (!sanitizedPath.startsWith(rootFolder)) {
                throw new RuntimeException(String.format("Path traversal attempt: %s (%s)", sanitizedPath, rootFolder));
            }
