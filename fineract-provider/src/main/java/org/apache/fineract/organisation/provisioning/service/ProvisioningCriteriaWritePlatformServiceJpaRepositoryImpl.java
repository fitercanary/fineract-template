...
public class ProvisioningCriteriaWritePlatformServiceJpaRepositoryImpl implements ProvisioningCriteriaWritePlatformService {
	...
	public CommandProcessingResult updateProvisioningCriteria(final Long criteriaId, JsonCommand command) {
		try {
			...
			List<SavingsProduct> savingsProducts = this.provisioningCriteriaAssembler.parseSavingsProducts(command.parsedJson());
			final Map<String, Object> changes = provisioningCriteria.update(command, products, savingsProducts);
			if (!changes.isEmpty()) {
