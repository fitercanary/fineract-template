...
public abstract class AbstractLoanScheduleGenerator implements LoanScheduleGenerator {
    ...
    private LoanScheduleModel generateWithReschedule(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
        ...
        if (loanScheduleParams == null) {
            ...
            LoanSchedulePeriodData loanSchedulePeriodData = lastSchedule.toData();
            scheduleParams = LoanScheduleParams.createLoanScheduleParams(currency, Money.of(currency, chargesDueAtTimeOfDisbursement),
...
//            loanApplicationTerms.updatePricipal(Money.of(currency,loanSchedulePeriodData.principalLoanBalanceOutstanding()));
        ...
        // interest method .
        if (!scheduleParams.isPartialUpdate() && !loanApplicationTerms.isEqualAmortization()) {
            Money originalPrincipal = loanApplicationTerms.getPrincipal();
            loanApplicationTerms.updatePricipal(scheduleParams.getPrincipalToBeScheduled());
            Money totalInterestChargedForFullLoanTerm = loanApplicationTerms
                    .calculateTotalInterestCharged(this.paymentPeriodsInOneYearCalculator, mc);

            loanApplicationTerms.updateTotalInterestDue(totalInterestChargedForFullLoanTerm);

//            loanApplicationTerms.updatePricipal(originalPrincipal);
                        ...
                        if (applicableDate.isBefore(scheduledDueDate)) {
                            ...
                            if (unprocessed.isGreaterThanZero()) {
                                ...
                                if (!scheduleParams.getOutstandingBalance().isGreaterThan(unprocessed) && !loanApplicationTerms
                                    ...
                                    if (!principalInterestForThisPeriod.interest()
                                        ...
                                        LoanTransaction loanTransaction = LoanTransaction.repayment(null, unprocessed, null,
                                                transactionDate, null, DateUtils.getLocalDateTimeOfTenant(), null, false);
                                ...
                                LoanTransaction loanTransaction = LoanTransaction.repayment(null, unprocessed, null, scheduledDueDate, null,
                                        DateUtils.getLocalDateTimeOfTenant(), null, false);
    ...
    /**
     * reschedule next installments for restructure
     * @param mc
     * @param loanApplicationTerms
     * @param loan
     * @param holidayDetailDTO
    ...
    public LoanScheduleDTO rescheduleNextInstallmentsRestructure(final MathContext mc, final LoanApplicationTerms loanApplicationTerms, Loan loan,
        ...
        LoanScheduleParams loanScheduleParams = null;
        Collection<LoanScheduleModelPeriod> periods = new ArrayList<>();
        Collection<LoanScheduleModelPeriod> retainedPeriods = new ArrayList<>();
        final List<LoanRepaymentScheduleInstallment> retainedInstallments = new ArrayList<>();
        LoanRepaymentScheduleInstallment finalInstallment = null;
        ...
        if (rescheduleFrom != null) {
            ...
            final List<LoanRepaymentScheduleInstallment> processInstallmentsInstallments = fetchRetainedInstallments(
                    loan.getRepaymentScheduleInstallments(), rescheduleFrom, currency);
            final List<LoanRepaymentScheduleInstallment> newRepaymentScheduleInstallments = new ArrayList<>();
            //todo -keep track of paid installments
            for (LoanRepaymentScheduleInstallment installment : processInstallmentsInstallments) {
                if (installment.isObligationsMet()) {
                    installment.updateInstallmentNumber(instalmentNumber);
                    newRepaymentScheduleInstallments.add(installment);

                    final LoanScheduleModelPeriod loanScheduleModelPeriod = createLoanScheduleModelPeriod(
                            installment, installment.getTotalOutstanding(currency));
                    retainedPeriods.add(loanScheduleModelPeriod);
                } else {
            ...
            Money[] receivables = loan.retriveIncomeOutstandingTillDate(currentDate);
            ...
            finalInstallment.updateInstallmentNumber(0);
            finalInstallment.updatePrincipal(principalPortion.getAmount());
            finalInstallment.updateFeesCharged(feesPortion.getAmount());
            finalInstallment.updateInterestCharged(interestPortion.getAmount());
            finalInstallment.updateFromDate(DateUtils.getLocalDateOfTenant().minusMonths(1));
            finalInstallment.updateDueDate(DateUtils.getLocalDateOfTenant());

            newRepaymentScheduleInstallments.add(finalInstallment);
            ...
            if (!newRepaymentScheduleInstallments.isEmpty() && totalCumulativeInterest.isGreaterThanZero()) {
                ...
                loanScheduleParams = LoanScheduleParams.createLoanScheduleParamsForPartialUpdate(periodNumber, instalmentNumber,
                        ...
                        totalFeeChargesCharged, totalPenaltyChargesCharged, totalRepaymentExpected,
                        totalOutstandingInterestPaymentDueToGrace, reducePrincipal, principalPortionMap, latePaymentMap, compoundingMap,
                        uncompoundedAmount, disburseDetailMap, principalToBeScheduled.plus(interestPortion).minus(transactionAmount), outstandingBalance.plus(interestPortion).minus(transactionAmount), outstandingBalanceAsPerRest,
                        newRepaymentScheduleInstallments, recalculationDetails, loanRepaymentScheduleTransactionProcessor, scheduleTillDate,
                        currency, applyInterestRecalculation);
                retainedInstallments.addAll(newRepaymentScheduleInstallments);
                loanScheduleParams.getCompoundingDateVariations().putAll(compoundingDateVariations);
        ...
        Collection<LoanScheduleModelPeriod> repaymentPeriods = loanScheduleModel.getPeriods();
        LoanScheduleModelPeriod disbursementItePeriod = repaymentPeriods.iterator().next();
        repaymentPeriods.remove(disbursementItePeriod);

        periods.add(disbursementItePeriod);
        periods.add(liquidatedModelPeriod);
        periods.addAll(repaymentPeriods);
