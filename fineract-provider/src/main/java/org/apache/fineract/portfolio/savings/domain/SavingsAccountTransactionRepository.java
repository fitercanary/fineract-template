...
public interface SavingsAccountTransactionRepository
    ...
    @Query("select sat from SavingsAccountTransaction sat where (sat.typeOf = :typeOfWithdrawal) and sat.savingsAccount.id = :savingsId and sat.dateOf = :transactionDate")
    List<SavingsAccountTransaction> findByTransactionTypeAndAccountIdAndDate(@Param("typeOfWithdrawal") Integer typeOfWithdrawal, @Param("savingsId") Long savingsId,
            @Param("transactionDate") Date transactionDate);
}
